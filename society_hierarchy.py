...
class Citizen:
    ...
    def get_citizen(self, cid: int) -> Optional[Citizen]:
        ...
        if self.cid == cid:
            ...
            for i in self._subordinates:
                if i.get_citizen(cid) is not None:
                    return i.get_citizen(cid)
        return None
    ...
    def get_all_subordinates(self) -> List[Citizen]:
        ...
        for i in self.get_direct_subordinates():
            sub = merge(sub, [i])
            sub.append(i.get_all_subordinates())
            for j in sub:
    ...
    def get_closest_common_superior(self, cid: int) -> Citizen:
        ...
        if self._superior is None:
            ...
            for i in self.get_all_subordinates():
                if i.cid == cid:
                    return self
    ...
    def get_district_name(self) -> str:
        ...
        """
        # Note: This method must call itself recursively
        if isinstance(self, DistrictLeader) and self is not None:
...
class Society:
    ...
    def get_all_citizens(self) -> List[Citizen]:
        ...
        True
        """
        all_citizens = self.get_head().get_all_subordinates()
        all_citizens = merge(all_citizens, [self.get_head()])
        return all_citizens
    ...
    def add_citizen(self, citizen: Citizen, superior_id: int = None) -> None:
        ...
        True
        """
        if citizen.get_superior() is None:
            if self.get_head() is None:
                self.set_head(citizen)
            elif self.get_head().get_citizen(superior_id) is not None:
                citizen.become_subordinate_to(
                    self.get_head().get_citizen(superior_id))
    ...
    def change_citizen_type(self, cid: int,
        ...
        if citizen is not None:
            if district_name is not None:
                newcitizen = DistrictLeader(cid, citizen.manufacturer,
                                            citizen.model_year,
                                            citizen.job, citizen.rating,
                                            district_name)
            else:
                newcitizen = Citizen(cid, citizen.manufacturer,
                                     citizen.model_year,
                                     citizen.job, citizen.rating)
            newcitizen.set_superior(citizen.get_superior())
            for i in subs:
                i.become_subordinate_to(newcitizen)
            if newcitizen.get_superior() is not None:
                newcitizen.get_superior().remove_subordinate(cid)
                newcitizen.get_superior().add_subordinate(newcitizen)
            else:
                self.set_head(newcitizen)
            return newcitizen
        return None
    ...
    def _swap_up(self, citizen: Citizen) -> Citizen:
        ...
        if sup_superior is not None:
            ...
            superior.remove_subordinate(citizen.cid)
            superior.become_subordinate_to(citizen)
        c_sub = citizen.get_direct_subordinates()
        s_sub = superior.get_direct_subordinates()
        for i in c_sub:
            if i != superior:
                i.become_subordinate_to(superior)
                citizen.remove_subordinate(i)
        for i in s_sub:
            if i != citizen:
                i.become_subordinate_to(citizen)
                superior.remove_subordinate(i)
        return citizen
    ...
    def promote_citizen(self, cid: int) -> None:
        ...
        if citizen is not None:
            if citizen.get_superior() is None:
                pass
            elif citizen.get_superior().rating > citizen.rating or \
                    isinstance(citizen, DistrictLeader):
                pass
            else:
                self.promote_citizen(self._swap_up(citizen).cid)
    ...
    def delete_citizen(self, cid: int) -> None:
        ...
        if citizen == self.get_head():
            ...
            for i in citizen.get_direct_subordinates():
                i.become_subordinate_to(citizen.get_superior())
...
class DistrictLeader(Citizen):
    ...
    def get_district_citizens(self) -> List[Citizen]:
        ...
        True
        """
        citizens = self.get_all_subordinates()
        for i in citizens:
            if i > self:
                citizens.insert(citizens.index(i), self)
                break
        return citizens
